...
case class PolyRedemption(polysToRedeem: IndexedSeq[(PublicKey25519Proposition, Nonce)],
                          signatures: IndexedSeq[Signature25519],
                          fee: Long,
                          timestamp: Long)
  extends BifrostTransaction {
  ...
  lazy val hashNoNonces = FastCryptographicHash(
    polysToRedeem.map(_._1.pubKeyBytes).reduce(_ ++ _) ++
    unlockers.map(_.closedBoxId).reduce(_ ++ _) ++
    Longs.toByteArray(timestamp) ++
    Longs.toByteArray(fee)
  )
  ...
  override lazy val json: Json = Map(
    ...
    "newBoxes" -> newBoxes.map(b => Base58.encode(b.id).asJson).asJson,
    "boxesToRemove" -> boxIdsToOpen.map(id => Base58.encode(id).asJson).asJson,
    "polysToRedeem" -> polysToRedeem.map {p =>
      Map(
        "proposition" -> Base58.encode(p._1.pubKeyBytes).asJson,
        "nonce" -> p._2.toString.asJson
      ).asJson
    }.asJson,
    "signatures" -> signatures.map(s => Base58.encode(s.signature).asJson).asJson,
    "fee" -> fee.asJson,

...
/**
  ...
  * @param u  H(m)&#94;k
  */
case class DiscreteLogProposition(l: Int, m: BigInt, g: BigInt, v: BigInt, u: BigInt) extends EqualityProposition[(BigInt, BigInt), BigInt, DiscreteLog] {
  ...
  def verify(proof: Proof[Proposition], sample: Array[Byte]): Boolean = proof match {
    case d: DiscreteLogProof =>
      d.c.equals(BigInt(DiscreteLogProof.hash(l=l, m, v, g^d.s / v^d.c, DiscreteLogProof.groupHash(m)^d.s/u^d.c))) &&
        sample.sameElements(DiscreteLogProof.hash(l=l, m, u))
    case _ => false
  }
}
